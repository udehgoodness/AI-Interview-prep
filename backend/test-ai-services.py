...
async def test_ai_service_with_fallback():
    """Test the AI service with fallback functionality"""
    logger.info("Testing AI service with fallback...")
    ...
    # Test with OpenAI
    result = await service.generate_interview_questions(job_title, job_description, model="openai")
    if result and "questions" in result:
        ...
        logger.error("Failed to generate questions with OpenAI")
    ...
    # Test with OpenAI
    evaluation = await service.evaluate_interview(sample_questions, sample_answers, job_title, model="openai")
    if evaluation:
        logger.info(f"Evaluation score using {evaluation.get('model_used', 'unknown')}: {evaluation['score']}")
        logger.info(f"Feedback: {evaluation['feedback'][:100]}...")
    else:
        logger.error("Failed to evaluate interview with OpenAI")
...
async def test_conversation_handling():
    """Test conversation handling with both services"""
    logger.info("Testing conversation handling...")
    ...
    if response:
        ...
        logger.error("Failed to handle conversation with OpenAI")
...
async def main():
    ...
    try:
        ...
        await test_conversation_handling()
